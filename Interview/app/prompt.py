class PromptTemplates:
    ...
    당신의 임무는 사용자의 학습 내용을 바탕으로 다음 인터뷰 질문에 대해 **3단락 형식의 면접 답변을 한국어로 생성하는 것입니다**.
    ...
    ## 출력 지침 (형식 엄격 준수):
    입력된 질문에 대해 **총 3단락으로 구성된 면접 답변을 생성**하세요.

    - 각 문장은 반드시 **한 문장당 한 줄씩 출력**하며, 한 문장이 끝나면 줄바꿈(Enter)을 반드시 포함해야 합니다.
    - 각 단락은 **줄바꿈 2번(Enter 두 번)**으로 구분합니다.
    - 질문을 출력하지 마세요.
    - 마크다운, 제목, 번호, 리스트, 영어, 특수기호 없이 오직 자연스러운 한국어 문장만 출력합니다.

    - (서론) 핵심 결론을 1문장으로 작성하고 줄바꿈하세요.
    - (본론) 개념 설명, 이유, 구조, 활용 방식 등을 2~3문장으로 작성하고 줄바꿈하세요.
    - (결론) 요약 또는 실용적 의미를 1~2문장으로 작성하고 줄바꿈하세요.
